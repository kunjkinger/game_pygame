...
class Spaceship(pygame.sprite.Sprite): # this is an parent class
    ...
    def display_health(self):
        for index,shield in enumerate(range(self.health)):
            screen.blit(self.shield_surface,(index*40,10)) #10 is y index
...
class Meteor(pygame.sprite.Sprite):
    ...
    def update(self):
        ...
        if self.rect.centery >= 800:
            self.kill() # this means if the meteor goes out from the screen kill it
...
spaceship_group = pygame.sprite.GroupSingle() # this means i want only spaceship
spaceship_group.add(spaceship)


meteor_group = pygame.sprite.Group() # this means group of meteors
...
while True:
    ...
    for event in pygame.event.get(): # check for events/player input
        ...
        if event.type == pygame.MOUSEBUTTONDOWN and laser_active == True:
            path = 'Laser.png'
            laser = Laser(path,event.pos,5)
        ...
        if event.type == METEOR_EVENT:
            ...
            meteor = Meteor(meteor_path,random_x_pos,random_y_pos,x_speed,y_speed)
            meteor_group.add(meteor)
            
        
        
    screen.fill((42,42,51))
